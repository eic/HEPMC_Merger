...
class signal_background_merger:
    ...
    def digestArgs(self):
        ...
        parser.add_argument('-i','--signalFile', default='pythia_DIS.hepmc',
        ...
        parser.add_argument('-bg1','--bg1File', default='pythia_PROTON.hepmc',
        ...
        parser.add_argument('-bg2','--bg2File', default='pythia_ELECTRON.hepmc',
        ...
        parser.add_argument('-bg3','--bg3File', default='pythia_SR.hepmc',
        ...
        parser.add_argument('--atLeastOne', action='store',type=bool, default=False,
                            help='Assert that at least one Background Event will always be admixed')
    ...
    def poissonTimes( self, mu, endTime ):
        ...
        while True :
            ...
            if t >= endTime :
                break
            ret = np.append(ret,t)
        if self.args.atLeastOne:
            if ret.size==0:
                while ret.size==0:
                    delt = self.rng.exponential( mu )
                    if delt<endTime:
                        ret=np.append(ret,delt)
        return ret
